...
#encText = "" #UNITEDSTATES
...
#finds and prints the most probable len of the key
def getKeylen():
	candidate = []
	c = 0
	for k in range(1,totalcount):
		...
		if(k%100==0):
			print(k,"/",totalcount)
		for i in freq:
			(key,rank) = deriveKey(k,i)
			totalrank += rank
		totalrank = totalrank/k
		if(totalrank > 2.0):
			candidate.append((k,totalrank))
			c+=1
			if c>2:
				break
			
	print("Candidate keylengths:")
	print("(keylen, Rankpoints)")
	print(candidate)
...
totalrank = 0
for i in freq:
	(key,rank) = deriveKey(keylen,i)
	totalrank += rank
	print(key)
print("Total Rankpoints:",totalrank/keylen)
